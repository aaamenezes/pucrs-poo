...
public class ListArrayOfInteger {
    ...
    /**
     * Adiciona um elemento no index especificado.
     * @param element elemento a ser adicionado
     */
    public void add(int index, Integer element) { // O(n)
        if (index > count) {
            // se tiver muito a frente e for ficar espaço vazio, negar
            System.out.println("Não pode inserir tão à frente");
            return;
        }

        if (index == count) {
            // se for igual a count, basta inserir
            add(element);
        }

        // se tiver ocupado, mover tudo pra frente, inserir novo elemento
        // pegar o último e colocar uma posição à frente (dobrar capacidade?)
        // pegar penúltimo e colocar mais uma posição à frente
        // fazer isso até chegar na posição index
        // inserir element nessa posição
    }
